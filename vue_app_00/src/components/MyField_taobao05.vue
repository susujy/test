<template>
  <div class="container">
    ...
    <!-- 淘宝logo图片 -->
    <div class="imgDiv">
      <img class="logo" src="../assets/tblogo.png" alt="">
    </div>
    <!-- 操作区域 -->
    <div class="operateDiv">
      <!-- 账户 -->
      <mt-field v-model="uname" :placeholder="accountPlaceholder"></mt-field>
      <!-- 密码 -->
      <mt-field  v-show="isUserAccount" type="password" v-model="upass" class="myinput" placeholder="请输入密码"></mt-field>
      <!-- 短信验证码 -->
      <mt-field v-show="!isUserAccount" placeholder="获取手机验证码" class="myinput"></mt-field>
      <!-- 免费注册 -->
      <!-- 登录和切换登录方式 -->
      <div>
        <mt-button size="large" class="mybutton" @click="login">登录</mt-button>
        <mt-button type="primary" size="large" class="mybutton" @click="changeLoginMethod">{{defaultAccountText}}</mt-button>  
      </div>
    </div>
  </div>
</template>
